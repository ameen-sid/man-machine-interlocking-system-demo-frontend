        ...
        <header className="h-16 border-b border-white/5 bg-industrial-black/80 backdrop-blur-sm sticky top-0 z-30 px-4 md:px-8 flex items-center justify-between">
          <div className="flex items-center gap-3 md:gap-4">
            ...
            <div className="hidden sm:flex items-center gap-4 text-[10px] md:text-xs tracking-widest text-gray-500 uppercase">
              ...
              <span className="hidden md:inline">Secure Protocol Active</span>
          ...
          <div className="flex items-center gap-4 md:gap-6">
                  ...
                  <>
                    ...
                    <div className="absolute right-0 mt-2 w-80 bg-industrial-black border border-white/10 rounded-2xl shadow-2xl z-50 overflow-hidden">
                      <div className="p-4 border-b border-white/5 flex items-center justify-between bg-white/5">
                        <h4 className="text-xs font-black uppercase tracking-widest text-industrial-accent">System Alerts</h4>
                        <span className="text-[9px] font-mono text-gray-500">{alerts.length} LOGS</span>
                      </div>
                      <div className="max-h-[400px] overflow-y-auto custom-scrollbar">
                        {alerts.map((alert) => (
                          <div key={alert.id} className="p-4 border-b border-white/5 hover:bg-white/5 transition-colors group">
                            <div className="flex gap-3">
                              <div className={`mt-1 shrink-0 w-1.5 h-1.5 rounded-full ${alert.type === 'error' ? 'bg-red-500' : alert.type === 'warning' ? 'bg-yellow-500' : 'bg-blue-500'}`} />
                              <div>
                                <p className="text-[11px] font-medium leading-normal">{alert.message}</p>
                                <p className="text-[9px] font-mono text-gray-500 mt-1 uppercase">
                                  {alert.timestamp instanceof Date ? alert.timestamp.toLocaleTimeString() : String(alert.timestamp)}
                                </p>
                              </div>
                            </div>
                          </div>
                      ...
                      <div className="p-3 bg-white/5 text-center border-t border-white/5">
                        <button className="text-[10px] font-black uppercase tracking-widest text-gray-400 hover:text-white transition-colors">Clear All Logs</button>
                      </div>
                    </div>
                  </>
                )}
             ...
             <div className="text-right">
                <p className="text-[9px] md:text-[10px] text-gray-500 leading-tight">SYSTEM TIME</p>
