...
const DUMMY_OPERATORS = [
	{
		...
		name: "John Doe",
		level: 3,
		certifications: ["CNC Milling", "Safety-L3", "Maintenance"],
	},
	{
		id: "OP-02",
		name: "Jane Smith",
		level: 2,
		certifications: ["Quality Control", "Assembly-L2"],
	},
	{
		id: "OP-03",
		name: "Mike Ross",
		level: 1,
		certifications: ["Safety-L1", "Manual Lathe"],
	},
	{
		id: "OP-04",
		name: "Sarah Connor",
	...
	},
	{
		...
		name: "David Chen",
		level: 2,
		certifications: ["Electronics", "PCB Soldering", "ESD Safety"],
	},
];
		...
		<div className="space-y-8">
			<div>
				<h2 className="text-2xl font-black uppercase tracking-tight">
					Skill{" "}
					<span className="text-industrial-accent">Mapping</span>
				</h2>
				<p className="text-gray-500 text-sm font-mono uppercase">
					Workforce Competency & Certification Matrix
				</p>
			</div>
						...
						<div className="flex flex-col sm:flex-row justify-between items-start gap-4">
							...
							<div className="flex gap-1">
								{[...Array(3)].map((_, i) => (
									<Star
										key={i}
										className={`w-3 h-3 sm:w-4 sm:h-4 ${i < op.level ? "text-industrial-accent fill-industrial-accent" : "text-gray-800"}`}
									/>
								))}
							</div>
						</div>
						...
						<div className="space-y-3">
							...
							<div className="flex flex-wrap gap-2">
								{op.certifications.map((cert) => (
									<span
										key={cert}
										className="px-3 py-1 bg-white/5 rounded-lg border border-white/5 text-[10px] text-gray-300 font-mono">
										{cert}
									</span>
								))}
							</div>
				...
				<div className="space-y-4">
					<div className="p-4 bg-red-500/5 rounded-xl border border-red-500/10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
						<span className="text-xs font-bold uppercase tracking-wide">
							CNC Multi-Axis Protocol
						</span>
						<span className="px-2 py-1 bg-red-500 text-black font-black text-[9px] rounded uppercase w-fit">
							Critical Gap
						</span>
					</div>
					<div className="p-4 bg-white/5 rounded-xl border border-white/5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 opacity-50">
						...
						<span className="px-2 py-1 bg-green-500/20 text-green-500 font-black text-[9px] rounded uppercase w-fit">
							Surplus
						</span>
					</div>
				</div>
