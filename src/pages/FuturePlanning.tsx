  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-black uppercase tracking-tight">Future <span className="text-industrial-accent">Planning</span></h2>
          <p className="text-gray-500 text-[10px] sm:text-sm font-mono uppercase">AI-Simulated Demand Forecasting & Capacity Mapping</p>
        </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
               ...
               <div className="flex flex-wrap gap-4 text-[10px] font-mono">
                  <div className="flex items-center gap-2">
                     ...
                     <span className="text-gray-500 uppercase">Demand</span>
                  </div>
                  <div className="flex items-center gap-2">
               ...
               <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={activeData}>
                     ...
                     <Tooltip 
                        ...
                        itemStyle={{ color: '#FCA311', fontSize: '12px' }}
                     />
                     <Area type="monotone" dataKey="demand" stroke="#FCA311" fillOpacity={1} fill="url(#colorDemand)" strokeWidth={3} />
                     <Area type="stepAfter" dataKey="capacity" stroke="rgba(255,255,255,0.1)" fill="none" strokeDasharray="5 5" />
                  </AreaChart>
               </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className="space-y-6">
            <div className="glass-card p-6 rounded-2xl border border-white/5 space-y-4">
               <div className="flex items-center gap-3 text-industrial-accent">
                  ...
                  <span className="text-[10px] font-bold uppercase tracking-widest">AI Strategy</span>
               </div>
            ...
            <div className="glass-card p-6 rounded-2xl border border-white/5 space-y-4 relative overflow-hidden group">
               ...
               <div className="flex items-center gap-3 text-gray-500">
                  ...
                  <span className="text-[10px] font-bold uppercase tracking-widest">Growth Index</span>
               </div>
               <div>
                  <p className="text-3xl font-mono font-bold">{view === '6m' ? '+24.8%' : '+112.4%'}</p>
                  <p className="text-[10px] text-green-500 font-bold uppercase tracking-tighter mt-1">Projected {view === '6m' ? 'Expansion' : '2-Year Alpha'}</p>
               </div>
            </div>
          </div>
      </div>
      ...
      <div className="glass-card p-4 sm:p-8 rounded-2xl border border-white/5">
         <h3 className="text-sm font-bold uppercase tracking-widest text-gray-400 mb-6">Strategic Milestones</h3>
         <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            {[
               { date: 'JUN 2026', task: 'Line-05 Hardware Install', status: 'Pending' },
               { date: 'JUL 2026', task: 'Operator L3 Training Cohort', status: 'Planned' },
               { date: 'SEP 2026', task: 'Global Supply Chain Sync', status: 'On-Hold' },
               { date: 'OCT 2026', task: 'System-Wide Firmware v5.0', status: 'Planned' },
            ].map((m) => (
               <div key={m.task} className="p-4 bg-white/5 rounded-xl border border-white/5 space-y-2">
                  <p className="text-[9px] font-mono text-industrial-accent">{m.date}</p>
                  <p className="text-xs font-bold leading-tight uppercase">{m.task}</p>
                  <p className="text-[9px] text-gray-600 uppercase font-black tracking-tighter">{m.status}</p>
               </div>
            ))}
         </div>
      </div>
