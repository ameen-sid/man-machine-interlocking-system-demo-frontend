...
export const UserManagement: React.FC = () => {
  ...
  const [formData, setFormData] = useState({ username: '', password: '', role: 'operator' as UserData['role'] });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingUser) {
        await api.put(`/users/${editingUser.id}`, { username: formData.username, role: formData.role });
      } else {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4">
        {loading ? (
          <div className="text-center py-10 text-slate-500 uppercase tracking-widest text-xs animate-pulse">Synchronizing Security Directory...</div>
        ) : (
            ...
            >
              <div className="flex items-center gap-4">
                <div className={`p-3 rounded-xl ${
                  ...
                  user.role === 'manager' ? 'bg-amber-500/10 text-amber-500' : 'bg-blue-500/10 text-blue-500'
                }`}>
                  {user.role === 'admin' ? <Shield size={24} /> : <User size={24} />}
                </div>
                <div>
                  <h3 className="text-lg font-semibold text-white">{user.username}</h3>
                  <span className={`text-[10px] px-2 py-0.5 rounded-full uppercase font-black tracking-widest ${
                    user.role === 'admin' ? 'bg-rose-500/20 text-rose-500' :
                    user.role === 'manager' ? 'bg-amber-500/20 text-amber-500' : 'bg-blue-500/20 text-blue-500'
                  }`}>
                    {user.role}
                  </span>
                </div>
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-1">
                ...
                >
                  <option value="operator">Operator (Basic)</option>
                  <option value="manager">Manager (Production Control)</option>
              ...
              >
                <Check size={20} />
                {editingUser ? 'Update Credentials' : 'Create Access Account'}
              </button>
            </form>
          </motion.div>
        </div>
