...
export const StationManagement: React.FC = () => {
	...
	const handleDelete = async (id: string) => {
		if (
			!window.confirm(
				"Delete this station? This may affect historical logs.",
			)
		)
			return;
		try {
			...
			<header className="flex justify-between items-center">
				<div>
					...
					<p className="text-slate-400 mt-1">
						Configure and monitor production stations
					</p>
				</div>
				<button
					onClick={() => {
				...
				{loading ? (
					...
					stations.map((station, index) => (
						<motion.div
							...
							<div
								className={`absolute top-0 right-0 w-32 h-32 -mr-8 -mt-8 blur-3xl opacity-20 transition-colors ${
							...
							<div className="space-y-4 relative z-10">
								...
								<div className="pt-4 border-t border-slate-800 flex justify-between items-center text-sm">
									...
									<span className="text-white font-bold bg-slate-800 px-2 py-1 rounded-lg">
										L{station.required_level}
									</span>
								</div>
							</div>
						</motion.div>
