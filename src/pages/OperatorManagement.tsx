...
import { Users, UserPlus, Trash2, Edit2, X, Check, Award, Shield } from 'lucide-react';
...
export const OperatorManagement: React.FC = () => {
  ...
  const [formData, setFormData] = useState({ id: '', name: '', level: 1, certifications: '' });
  ...
  return (
    <div className="p-6 space-y-6">
      <header className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-white tracking-tight">Operator Registry</h1>
          <p className="text-slate-400 mt-1">Manage personnel certifications and skill levels</p>
        </div>
        <button 
          onClick={() => {
            setEditingOp(null);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (
          <div className="col-span-full text-center py-20 text-slate-500 animate-pulse uppercase tracking-[0.3em] text-xs">Accessing Personnel Database...</div>
        ) : (
            ...
            >
                  ...
                  <button 
                    onClick={() => {
                      ...
                      setFormData({ 
                        ...
                        name: op.name, 
                        level: op.level, 
                        certifications: getCerts(op).join(', ') 
                      });
                      setIsModalOpen(true);
                    }}
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <span className="text-xs text-slate-500 uppercase font-bold tracking-wider">Skill Maturity</span>
                  <div className="flex items-center gap-1">
                    ...
                    <span className="ml-2 text-xs font-black text-blue-400">L{op.level}</span>
                  </div>
                </div>

                <div className="space-y-2">
                  <span className="text-[10px] text-slate-500 uppercase font-black tracking-[0.2em]">Validated Certs</span>
                  <div className="flex flex-wrap gap-2">
                    {getCerts(op).map((cert: string) => (
                      <span key={cert} className="px-2 py-1 bg-slate-800 text-slate-300 text-[9px] rounded-md border border-slate-700 flex items-center gap-1">
                        <Award size={10} className="text-blue-500" />
                        {cert}
                      </span>
                    ))}
                    {getCerts(op).length === 0 && <span className="text-[10px] text-slate-600 italic">No certifications recorded</span>}
                  </div>
                </div>
              </div>
            </motion.div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-bold text-white">{editingOp ? 'Update Personnel' : 'New Operator Registration'}</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Skill Level (L1 - L3)</label>
                <select 
                  className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-2 text-white focus:outline-none"
                  value={formData.level}
                  onChange={e => setFormData({...formData, level: parseInt(e.target.value)})}
                >
                  <option value={1}>L1 - Beginner (Limited Access)</option>
                  <option value={2}>L2 - Proficient (Standard Access)</option>
                  <option value={3}>L3 - Expert (Mastery/Supervision)</option>
                </select>
              </div>

              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Certifications (Comma separated)</label>
                <textarea 
              ...
              >
                <Check size={20} />
                {editingOp ? 'Commit Updates' : 'Finalize Registration'}
              </button>
            </form>
          </motion.div>
        </div>
