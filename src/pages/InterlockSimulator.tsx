...
export const InterlockSimulator: React.FC = () => {
	...
	const handleSimulateScan = () => {
		if (!selectedOperator || !selectedMachine) {
			setAuthStatus({
				type: "error",
				message: "Please select operator and machine",
			});
			return;
		}
		...
		<div className="space-y-8 min-h-full flex flex-col items-center py-8 md:justify-center md:py-0 md:-mt-16">
			<div className="text-center space-y-2 mb-4">
				<h2 className="text-2xl md:text-3xl font-black tracking-tight uppercase">
					Interlock{" "}
					<span className="text-industrial-accent underline decoration-industrial-accent/30 underline-offset-8">
						Unit-01
					</span>
				</h2>
				<p className="text-gray-500 font-mono text-[10px] md:text-sm">
					HMI PROTOCOL: SECURE-RFID v4.2
				</p>
			</div>
					...
					<AnimatePresence mode="wait">
						{isScanning ? (
							<motion.div
								...
								<p className="text-industrial-accent font-mono animate-pulse">
									ENCRYPTED AUTHENTICATION IN PROGRESS...
								</p>
							</motion.div>
							...
							<motion.div
								...
								<div className="space-y-1">
									...
									<p className="text-gray-500 text-sm max-w-[200px]">
										Please select operator credentials and
										target hardware to initiate.
									</p>
								</div>
							</motion.div>
							...
							<motion.div
								...
								<p className="text-[10px] text-gray-500 uppercase tracking-widest leading-loose">
									Incident logged in master registry.
									<br />
