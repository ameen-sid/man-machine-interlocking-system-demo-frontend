		...
		<div className="space-y-8">
			<div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
				<div>
					<h2 className="text-2xl font-black uppercase tracking-tight">
						Shift{" "}
						<span className="text-industrial-accent">Reports</span>
					</h2>
					<p className="text-gray-500 text-[10px] sm:text-sm font-mono uppercase">
						Multi-Shift Yield & Quality Performance Logs
					</p>
				</div>
				...
				{SHIFT_DATA.map((s) => (
					<div
						...
						<div className="flex justify-between items-start">
							...
							<span
								className={`text-[10px] font-black ${s.output >= s.target * 0.95 ? "text-green-500" : "text-yellow-500"}`}>
								{((s.output / s.target) * 100).toFixed(1)}%
								TARGET
							</span>
						</div>
						<div className="space-y-1">
							...
							<div className="w-full h-1 bg-white/5 rounded-full overflow-hidden">
								<div
									...
									style={{
										width: `${Math.min(100, (s.output / s.target) * 100)}%`,
									}}
								/>
							</div>
						</div>
			...
			<div className="p-8 glass-card rounded-2xl border border-white/5 space-y-6">
				<h3 className="text-sm font-bold uppercase tracking-widest text-gray-400">
					Shift Highlights & Comments
				</h3>
				<div className="space-y-4">
					<div className="p-4 bg-white/5 rounded-xl border-l-4 border-industrial-accent">
						<p className="text-xs font-bold mb-1">
							High Throughput on CNC-01 (Shift A)
						</p>
						<p className="text-[10px] text-gray-500 italic">
							"Achieved 102% of target due to optimized tooling
							sequence." - Supervisor A
						</p>
					</div>
					<div className="p-4 bg-white/5 rounded-xl border-l-4 border-red-500">
						<p className="text-xs font-bold mb-1">
							Power Fluctuations (Shift B)
						</p>
						<p className="text-[10px] text-gray-500 italic">
							"Minor lag in Line-04 attributed to grid instability
							at 19:40." - Supervisor B
						</p>
					</div>
				</div>
			</div>
